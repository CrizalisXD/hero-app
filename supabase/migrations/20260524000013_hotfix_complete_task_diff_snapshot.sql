-- ════════════════════════════════════════════════════════════════════
-- Hero — 0016_hotfix_complete_task_diff_snapshot.sql
--
-- The previous hotfix (0015) tried to capture trigger-fired unlocks via
-- clock_timestamp() comparison, but user_achievements.unlocked_at uses
-- DEFAULT now() which returns transaction_timestamp() — always EARLIER
-- than clock_timestamp() called later in the same txn. So the filter
-- `unlocked_at >= v_started_at` matched nothing.
--
-- New approach: snapshot the set of currently-held achievement_ids at
-- the start of the RPC, then return the SET DIFFERENCE at the end. This
-- correctly captures both:
--   1) direct check_and_unlock_achievements(...) calls inside the RPC
--   2) trigger-fired unlocks (challenge_completed via tg_tl_challenge)
--
-- No assumption about timestamps — pure set algebra.
-- ════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Snapshot achievement_ids the user already had before this RPC.
  ...
  -- Firing tg_tl_challenge here may insert challenge_complete into
  -- user_achievements as a side effect.
  ...
  -- Diff against snapshot: return achievements newly added during this RPC.
